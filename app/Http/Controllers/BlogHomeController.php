...
class BlogHomeController extends Controller
{
   ...
   public function react_forum(Request $req)
   {
      if ($req->session()->get('id')) {
         ...
         $id_auth =$req->session()->get('id');
         $user_react = DB::table('react')->join('accounts','react.id_auth_cmt','=','accounts.id')->where('accounts.id',$id_auth)->where('react.id_post',$id_post)->get();
         if (count($user_react)) {
            $delReact = React::where('id_auth_cmt',$id_auth)->delete();
            echo 'Thích[ ';
         }else{
            $addReact = new React;
            $addReact->id_auth_cmt=$id_auth;
